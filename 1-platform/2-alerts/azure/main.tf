...
# Tags: root generates zz_autogenerated_tags.tf (common_tags, tag_globals, tag_context → local.rendered_tags). Use it for action group; build per-rule-group tags like 1-k8s.
...
locals {
  ...
  # Per-resource rendered tags for each rule group (tag_context from terragrunt has resource_name for action group → use local.rendered_tags for that).
