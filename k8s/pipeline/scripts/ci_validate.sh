...
usage() {
  cat <<'EOF'
Usage:
  ci_validate.sh --provider <gcp|azure> --values-dir <path> [options]
...
Example:
  ./ci_validate.sh \
    --provider gcp \
    --values-dir ./helm-values \
    --helmfile ./helmfile.yaml.gotmpl \
    --cluster-name divyam-gke-prod-1-asia-south1 \
    --region asia-south1 \
...
echo "=== CI validate: provider=$PROVIDER values=$VALUES_DIR helmfile=$HELMFILE_FILE ==="
...
pipeline_run "helmfile -f \"$HELMFILE_FILE\" diff"

echo "CI validation successful: helmfile diff completed."
