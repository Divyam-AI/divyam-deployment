...
The platform ships several stacks: the **router** ecosystem (router-controller, route-selector,
selector-training, evaluator, clickhouse, kafka, superset, and so on), the **evalm8** stack (evalm8
apps with their temporal, lakefs, and argilla datastores and operators), and **self-serve**.
`external-secrets`, `mysql`, and persistent storage are shared and always deploy regardless of the
selection.

Every chart belongs to exactly one stack. A cluster that runs several composes them by naming
several — a self-serve cluster runs the core *and* the self-serve services, so it names all three.

The selection comes solely from the `stack` key in `provider.yaml`, which terraform generates from
the cluster's terragrunt `stack` value:

```yaml
# provider.yaml (generated by terraform — do not hand-edit; the next regen overwrites it)
stack: router,evalm8              # a list drawn from router | evalm8 | self-serve
stack: all                        # every chart; also what an absent key means
```

There is deliberately **no env var and no CLI flag**. The same terragrunt value gates whether each
stack's cloud resources are procured at all — the evalm8 object storage, its secret-manager keys,
its service accounts — so charts and infrastructure must be selected by one value or they can
disagree, and a cluster can be asked to run charts whose backing resources were never created. To
change what a cluster deploys, change its terragrunt `stack` and re-run the export.

To deploy part of a cluster's stack, target releases instead: `-l <chart>` (repeatable) selects
individual charts without claiming anything about the cluster's composition. A chart outside the
selected stacks is omitted from the release list rather than uninstalled, so a narrowed deploy never
removes what another stack has already installed.
