...
dependency "azure_blob_storage" {
  ...
  mock_outputs = {
    router_requests_logs_storage_account_connection_string = "mock-connection-string"
    router_requests_logs_storage_account_name              = "mock-storage-account"
    router_requests_logs_container_names                   = ["mock-container"]
  }
}
...
inputs = merge(local.merged_inputs, {
  ...
  azure_router_logs_storage_connection_string = dependency.azure_blob_storage.outputs.router_requests_logs_storage_account_connection_string
  azure_router_logs_storage_account_name = dependency.azure_blob_storage.outputs.router_requests_logs_storage_account_name

  # TODO: Using the first container as the logs container.
  azure_router_logs_container_name            = dependency.azure_blob_storage.outputs.router_requests_logs_container_names[0]
