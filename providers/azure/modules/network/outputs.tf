...
output "network_id" {
  description = "Standardized network ID (alias for vnet_id)"
  value       = (var.use_existing_vnet ? data.azurerm_virtual_network.vnet[0].id : azurerm_virtual_network.vnet[0].id)
}

output "network_name" {
  description = "Standardized network name (alias for vnet_name)"
  value       = (var.use_existing_vnet ? data.azurerm_virtual_network.vnet[0].name : azurerm_virtual_network.vnet[0].name)
}

output "network_self_link" {
  description = "Standardized network self-link (GCP-compatible alias)"
  value       = (var.use_existing_vnet ? data.azurerm_virtual_network.vnet[0].id : azurerm_virtual_network.vnet[0].id)
}
