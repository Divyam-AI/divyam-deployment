...
resource "azurerm_monitor_action_group" "alerts" {
  ...
  dynamic "webhook_receiver" {
    for_each = var.notification_zenduty_webhook_url == null ? [] : [var.notification_zenduty_webhook_url]
    content {
      name                    = "zenduty-webhook"
      service_uri             = webhook_receiver.value
      use_common_alert_schema = true
    }
  }
...
resource "azurerm_monitor_scheduled_query_rules_alert_v2" "no_logs_alert" {
  name                = "${var.resource_name_prefix}-no-logs-detected-alert"
  resource_group_name = var.resource_group_name
  location            = var.location

  scopes = ["/subscriptions/8645e690-451d-45a4-b10c-159705f63a22/resourcegroups/pre-production/providers/microsoft.operationalinsights/workspaces/dummy-log-workspace"]
  ...
  tags = {
    for key, value in local.common_tags :
    key => templatestring(value, {
      resource_name  = "${var.resource_name_prefix}-no-logs-detected-alert"
      location       = var.location
      resource_group = var.resource_group_name
      environment    = var.environment
    })
