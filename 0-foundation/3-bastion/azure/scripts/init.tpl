...
%{ if length(kube_config_map) > 0 ~}
echo "Installing kube-config for AKS clusters..."

# Create directories
mkdir -p /home/${admin_username}/.kube

# Generate kube-config for each AKS cluster
...
%{ else ~}
echo "No AKS cluster configured. Skipping kubeconfig setup."
echo "You can manually configure kubeconfig later using: az aks get-credentials"
%{ endif ~}
