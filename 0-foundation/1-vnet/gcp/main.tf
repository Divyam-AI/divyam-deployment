...
locals {
  network_id = var.vnet.create || var.import_mode ? google_compute_network.vpc[0].id : data.google_compute_network.vpc[0].id
}
...
resource "google_compute_shared_vpc_host_project" "host" {
  count = (var.vnet.create || var.import_mode) && try(var.vnet.shared_vpc_host, false) ? 1 : 0
