...
inputs = merge(
  {
    ...
    database_version              = try(local.cloudsql_postgres.database_version, "POSTGRES_16")
    tier                          = try(local.cloudsql_postgres.tier, "db-f1-micro")
    edition                       = try(local.cloudsql_postgres.edition, "ENTERPRISE")
