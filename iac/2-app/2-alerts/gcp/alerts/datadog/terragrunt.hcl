...
# Cloud-gated: this GCP unit and azure/datadog share the same datadog module and Datadog org.
# Without the cloud_provider guard a `**/datadog` run selects BOTH and creates duplicate monitors.
exclude {
  if      = !local.alerts_run || !local.datadog_enabled || try(local.root.cloud_provider, "") != "gcp"
